...
public class ProductInfoAction {
    ...
    public String split(HttpServletRequest request){
        ...
        if(vo != null ){
            ...
            //这里是将存在session里的信息删除，到这一步存的信息已经被用完了
    ...
    //异步ajax文件上传处理
    @ResponseBody
    @RequestMapping("/ajaxImg")//对应addproduct的19行
    public Object ajaxImg(MultipartFile pimage ,HttpServletRequest request)//这个pimage是对应addproduct的第81行name
    {
    ...
    @RequestMapping("/save")
    public String save(ProductInfo info, HttpServletRequest request) { //所有的信息都封装在这个ProductInfo实体类中
    ...
    @RequestMapping("/one")
    public String one(int pid,ProductInfoVo vo, Model model,HttpSession session){//加上session是因为要把vo里的查询数据存储起来，因为这里涉及到了页面的跳转，存到session里是最好的选择
    ...
    @RequestMapping("/delete")//这里不能加@ResponseBody因为还有没有处理结束？
    ...
    //传进来的是pids="1,4,5"  我们要转换为这样的[1,4,5]数组
    public String deleteBatch(String pids, HttpServletRequest request){
        ...
        try {
            ...
            request.setAttribute("msg","不可删除");//这里是防止被添加到购物车里的商品被删除
        }
        return "forward:/prod/deleteAjaxSplit.action";
    }
