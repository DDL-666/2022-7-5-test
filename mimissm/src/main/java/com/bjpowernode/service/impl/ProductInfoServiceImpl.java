...
public class ProductInfoServiceImpl implements ProductInfoService {
    ...
    public PageInfo splitPage(int pageNum, int pageSize) {
        ...
        //设置排序,按主键降序排序.(当你添加新数据时，在数据库中是添加到最后面的，不进行降序查询的话
        // 那么新数据也是在最后面的不利于查看)
        //select * from product_info order by p_id desc
        example.setOrderByClause("p_id desc");//使用ctrl+f在ProductInfoMapper.xml中可以找到这个方法(xml文件中是不带set的 且在85行)，加上"p_id desc"使查询语句完整
        //设置完排序后,取集合,切记切记:一定在取集合之前,设置PageHelper.startPage(pageNum,pageSize);
        List<ProductInfo> list = productInfoMapper.selectByExample(example);
        //将查询到的集合封装进PageInfo对象中
        PageInfo<ProductInfo> pageInfo = new PageInfo<>(list);
        //PageInfo和PageHelper是这个分页插件的主要类
        return pageInfo;
    }
