...
<script type="text/javascript">
    ...
    function allClick() {
        ...
        //将此状态赋值给下面五个复选框  （input标签里的name属性）
...
<script type="text/javascript">
    ...
    function deleteBatch() {
        ...
        if(cks.length == 0){
            ...
            var str = "";
            var pid = "";
            if(confirm("您确定要删除"+cks.length+"条商品吗?")){//confirm是一个Window对象
               // alert("可以进行删除!");
                //获取其value的值,进行字符串拼接
                $.each(cks,function () {
                    pid = $(this).val();
...
<script type="text/javascript">
    function ajaxsplit(page) {
        ...
        $.ajax({
            ...
            data: {"page": page,"pname":pname,"typeid":typeid,"lprice":lprice,"hprice":hprice},//将查询的条件一并发出去
            type: "post",
            success: function () {
                //重新加载显示分页数据的容器
                $("#table").load("http://localhost:8080/admin/product.jsp #table");
    ...
    function condition() {
        ...
        $.ajax({
            ...
            success:function () {
                //刷新显示数据的容器
                $("#table").load("http://localhost:8080/admin/product.jsp #table");<!--p70-->
            }
        });
    }
</script>
